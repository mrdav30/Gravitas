...
| `StiffBody2D` kinematic active-source CCD | same static-style and dynamic candidate paths above | frame-start to host-target 2D proxy circle or mixed slab proxy | first static-style target clips the source; dynamic 2D/3D targets up to that blocker receive wake plus positional correction | same reducer policy as the underlying pure or mixed path |
