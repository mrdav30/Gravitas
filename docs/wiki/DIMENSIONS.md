...
- 2D position is `Vector2d`; `Vector2d.x` maps to world X and `Vector2d.y`
  maps to world Z.
- World Y is not a 2D collision axis. In mixed mode it centers the finite
  2D slab used by cross-dimensional collision.
...
`SolidBody` is constructed from an `IMatterAgent` and an `LSCollider`. The
agent supplies the context and host transform bridge. Dynamic 3D bodies publish
authoritative `Vector3d` position and `FixedQuaternion` rotation back to the
host `FixedTransform` during `Visualize()` whenever the runtime mode runs the
3D service (`ThreeD`, `Both`, or `Mixed`). Kinematic 3D bodies read the agent
...
mutation wakes sleeping bound bodies before broad-phase refresh, matching the
2D mutation contract.
...
| Scalar rotation    | Yaw around world Y                 |
...
vertical Y value is preserved because it is not part of 2D physics.
...
2D compound assets should serialize `CompoundColliderPart2D` values, not
private runtime part colliders. `LSCompoundCollider2D` aggregates private parts
in stable part order, assigns area-proportional part mass, applies owner local
...
it is not public 3D thickness and does not imply mixed collision. The 2D
runtime does not expose a public bounds/config bridge; bounds ownership stays
on `LSCollider2D` and runtime services.
...
2D limits and motors use `Fixed64` scalar values. Distance limits are
world units, slider limits are signed translation along the joint axis, angular
limits are radians, angular motors target scalar local angle, and linear motors
target slider translation.

Enabled `Joint2D` rows solve inside the same 2D response islands as contact
...
| `MixedSlabCenterY`                     | Cached from the host `FixedTransform.Position.y`.     |
| `MixedBounds3D`                        | `FixedBoundBox` from 2D X/Z bounds plus the Y slab. |

The mixed 3D bounds do not change 2D collision truth or 2D partition
...
Mixed contact processing runs during `LateSimulate()` after both
dimension-local services have integrated bodies and refreshed their own collider
partitions.
...
queries gather GridForge-backed partition candidates, suppress duplicates,
apply layer masks, run exact 3D shape or mesh checks, and sort by deterministic
hit ordering.
...
`Query2D` and `Query3D` stay dimension-local. Mixed CCD uses `QueryMixed` only when
`PhysicsRuntimeMode.Mixed` is active, so `Both` remains isolated.
