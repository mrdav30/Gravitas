...
came from mixed-specific repeated target partition refresh removal, not from
the kinematic/static membership split alone. Pure 2D and pure 3D partitions
still store kinematic bodies in their dynamic ID sets, so this phase should
measure before changing those services.
...
- [ ] Verify public pure 2D/3D queries still include dynamic, kinematic,
  bodyless, and immovable colliders according to their existing query contract.
- [ ] Audit CCD proxy-radius behavior for sphere/circle, capsule, cuboid/AABB,
...
- [ ] Add focused tests for proxy-policy edge cases where the current
  conservative approximation could be misunderstood by hosts.
- [ ] Improve benchmark guardrails if short-iteration warnings remain noisy:
  increase operations per invoke, add a larger stress row, or document why a
  row is only directional evidence.
- [ ] Re-run focused CCD tests, full `Release`, full `ReleaseLean`, and the
