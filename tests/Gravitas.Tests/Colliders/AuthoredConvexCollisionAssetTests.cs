...
public sealed class AuthoredConvexCollisionAssetTests
{
    ...
    public void CompoundPartConstructor_ShouldApplyAuthoringTransformAtomically()
    {
        ...
        var descriptor = new CompoundColliderPart(part, localOffset, localRotation, localScale);

        descriptor.Collider.Should().BeSameAs(part);
    ...
    public void AuthoredConvexCompound_ShouldKeepPiecesBehindOneBroadPhaseIdentity()
    {
        ...
        var leftPiece = MeshTestFixtures.CreateConvexCube();
        var rightPiece = MeshTestFixtures.CreateConvexCube();
        var compound = new LSCompoundCollider(
            new CompoundColliderPart(leftPiece, new Vector3d(-Fixed64.One, Fixed64.Zero, Fixed64.Zero), FixedQuaternion.Identity, Vector3d.One),
            new CompoundColliderPart(rightPiece, new Vector3d(Fixed64.One, Fixed64.Zero, Fixed64.Zero), FixedQuaternion.Identity, Vector3d.One));
    ...
    {
        using PhysicsScenarioBuilder scenario = PhysicsScenarioBuilder.Create();
        LSMeshCollider meshPiece = MeshTestFixtures.CreateConvexCube();
        var compound = new LSCompoundCollider(
            new CompoundColliderPart(meshPiece, Vector3d.Zero, FixedQuaternion.Identity, Vector3d.One));
    ...
    public void Diagnostics_ShouldCaptureAuthoredConvexMeshPartsWithOwnerIdentity()
    {
        using PhysicsScenarioBuilder scenario = PhysicsScenarioBuilder.Create();
        var compound = new LSCompoundCollider(
            new CompoundColliderPart(MeshTestFixtures.CreateConvexCube(), Vector3d.Zero, FixedQuaternion.Identity, Vector3d.One));
        ScenarioBody<LSCompoundCollider> body = scenario.CreateBody(
            compound,
            Vector3d.Zero,
            FixedQuaternion.Identity,
